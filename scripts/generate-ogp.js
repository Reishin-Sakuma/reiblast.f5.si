 ...
 *
 * 使い方:
 *   node scripts/generate-ogp.js <slug>   # 特定記事のみ
 *   node scripts/generate-ogp.js --all    # 全記事（draft除く）
 */
...
if (args[0] === '--all') {
  ...
  console.log(`全${posts.length}記事のOGP画像を生成します...`);
  let ok = 0;
  ...
  console.log(`完了: ${ok}/${posts.length} 件`);
} else if (args[0]) {
  const slug = args[0];
  const posts = collectPosts({ includeDraft: true });
  ...
  console.error('使い方: node scripts/generate-ogp.js <slug> または --all');
  process.exit(1);
}

// ── メイン処理 ────────────────────────────────────────────────────────────

function generate({ slug, title, dir }) {
  const t0 = Date.now();
  const svg = buildOgpSvg(title);
  ...
  // 記事フォルダに ogp.png として保存
  const outPath = join(dir, 'ogp.png');
  writeFileSync(outPath, png);
  const rel = outPath.replace(ROOT + '/', '');
  console.log(`生成完了: ${rel} (${png.length} bytes, ${Date.now() - t0}ms)`);
}

function collectPosts({ includeDraft = false } = {}) {
  const posts = [];
  function walk(dir) {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        if (fm.slug && fm.title && (includeDraft || !fm.draft)) {
          posts.push({ slug: fm.slug, title: fm.title, dir: dirname(full) });
        }
      }
    }
  }
